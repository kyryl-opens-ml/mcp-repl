...
def get_apps():
    ...
    try:
        result = subprocess.run(["modal", "app", "list", "--json"], capture_output=True, text=True, check=True)
...
def read_process_output_with_timeout(command, timeout=3, max_lines=100):
    ...
    # Create a queue for collecting output
    output_queue = queue.Queue()
    
    # Function to read output and put it in the queue
...
def stop_container(container_id: str):
    ...
    try:
        ...
        if output_lines:
            return {"message": "Container stop command completed", "details": output_lines}
        else:
